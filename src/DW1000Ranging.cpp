...
uint16_t DW1000RangingClass::getShortAddr(const char *addr){
	uint8_t byteShortAddr[2];
	uint16_t shortAddress;
	pDW1000.convertToByte(addr, byteShortAddr, 2);
	shortAddress = (byteShortAddr[0] | uint16_t(byteShortAddr[1] << 8));
	return shortAddress;
}

void DW1000RangingClass::init(BoardType type, const char *wifiMacAddress, bool high_power, const uint8_t mode[], float payload)
{
	for(int i=0;i<MAX_DEVICES;i++) _networkDevices.push_back(DW1000Device(_portable));
	
	uint8_t byteWifiMacAddress[6] = {0};

	uint16_t shortAddress = getShortAddr(wifiMacAddress);
	...
	_handleNewRange = NULL;
	_handleRangeSent = NULL;
	_handleBlinkDevice = NULL;
	_handleNewDevice = NULL;
	_handleInactiveDevice = NULL;
	_handleRemovedDeviceMaxReached = NULL;
	_payload = payload;
	...
	_ownLongAddress[2] = byteWifiMacAddress[0];
	_ownLongAddress[3] = byteWifiMacAddress[1];
	_ownLongAddress[4] = byteWifiMacAddress[2];
	_ownLongAddress[5] = byteWifiMacAddress[3];
	_ownLongAddress[6] = byteWifiMacAddress[4];
	_ownLongAddress[7] = byteWifiMacAddress[5];
...
void DW1000RangingClass::generalStart(bool high_power)
{
	...
	pDW1000.attachSentHandler([&]() {  handleSent(); });
	pDW1000.attachReceivedHandler([&]() { handleReceived(); });
	...
	pDW1000.getPrintableDeviceIdentifier(msg);
	m_log::log_vrb(LOG_DW1000, "Device ID: %s", msg);

	pDW1000.getPrintableExtendedUniqueIdentifier(msg);
	...
	pDW1000.getPrintableNetworkIdAndShortAddress(msg);
	m_log::log_vrb(LOG_DW1000, "Network ID & Device Address: %s", msg);

	pDW1000.getPrintableDeviceMode(msg);
...
bool DW1000RangingClass::addNetworkDevices(DW1000Device *device)
{
	...
	{
		memcpy((void *)&_networkDevices[_networkDevicesNumber], device, sizeof(DW1000Device)); // FIXME
	...
	else
	{
		...
		if (_handleRemovedDeviceMaxReached != NULL)
		{
			_handleRemovedDeviceMaxReached(&_networkDevices[worstQuality]);
		}
		memcpy((void *)&_networkDevices[worstQuality], device, sizeof(DW1000Device)); // FIXME
...
void DW1000RangingClass::removeNetworkDevices(uint8_t index)
{
	if (index != _networkDevicesNumber - 1) // if we do not delete the last element
	{
		...
		memcpy((void *)&_networkDevices[index], &_networkDevices[_networkDevicesNumber - 1], sizeof(DW1000Device)); // FIXME
...
MessageType DW1000RangingClass::detectMessageType(uint8_t datas[])
{
...
void DW1000RangingClass::loop()
{
	...
	if (_sentAck)
	{
		...
		else if (_type == BoardType::TAG)
		{
			if (messageType == MessageType::POLL)
			{
				...
				DEBUGtimePollSent = _portable.millis();
	...
	if (_receivedAck)
	{
		...
		if (messageType == MessageType::BLINK && _type == BoardType::ANCHOR)
		{
			...
			DW1000Device myTag(_portable, shortAddress);
		...
		else
		{
				...
				if (messageType == MessageType::POLL)
				{
					...
					for (uint8_t i = 0; i < numberDevices; i++)
					{
							...
							shortAddress[1] == _ownShortAddress[1])
						{
							...
							pDW1000.getReceiveTimestamp(myDistantDevice->timePollReceived);
					...
					int randomSlot = _portable.random(0, pollAckTimeSlots - numberDevices);
				...
				else if (messageType == MessageType::RANGE)
				{
					...
					for (uint8_t i = 0; i < numberDevices; i++)
					{
						...
						if (shortAddress[0] == _ownShortAddress[0] && shortAddress[1] == _ownShortAddress[1])
						{
							...
							// we grab the replytime which is for us
							pDW1000.getReceiveTimestamp(myDistantDevice->timeRangeReceived);
							noteActivity();
							_expectedMsgId = MessageType::POLL;
							...
							if (!_protocolFailed)
							{
								...
								float payload; memcpy(&payload, receivedData + SHORT_MAC_LEN + 14 + rangeDeviceSize * i, 4);
			...
			else if (_type == BoardType::TAG)
			{
				...
				if (messageType == MessageType::POLL_ACK)
				{
					...
					// Serial.println(pDW1000.getReceivePower());
					// Serial.println(pDW1000.getFirstPathPower());
					// Serial.println(pDW1000.getReceiveQuality());
					...
					if (_replyTimeOfLastPollAck != 0 && myDistantDevice->getShortAddress() == _addressOfExpectedLastPollAck)
					{
						...
						DEBUGRangeSent = _portable.millis();
...
void DW1000RangingClass::copyShortAddress(uint8_t to[], uint8_t from[])
{
	to[0] = from[0];
	to[1] = from[1];
}
...
void DW1000RangingClass::transmitRangingInit(uint16_t delay)
{
	transmitInit();
...
void DW1000RangingClass::transmitPollAck(DW1000Device *myDistantDevice, uint16_t delay)
{
	transmitInit();
...
void DW1000RangingClass::transmitRangeReport(DW1000Device *myDistantDevice, uint16_t delay)
{
	transmitInit();
	_globalMac.generateShortMACFrame(sentData, _ownShortAddress, myDistantDevice->getByteShortAddress());
